...
// Expand a `cases` array into one sim entry per case.
// Each case is a partial JSON override merged on top of `base`. Returns an array of sim entries.
function getProductCases(base, cases) {
    const out = [];
    for (const c of cases) {
        const merged = JSON.parse(JSON.stringify(base));
        if (c && c.players) {
            merged.players = Object.assign({}, merged.players, c.players);
        }
        if (c && c.zone) merged.zone = c.zone;
        if (c && c.difficultyTier !== undefined) merged.difficultyTier = c.difficultyTier;
        if (c && c.name) merged.name = c.name;
        if (c && c.simulationTimeLimit) merged.simulationTimeLimit = c.simulationTimeLimit;
        out.push(merged);
    }
    return out;
}
